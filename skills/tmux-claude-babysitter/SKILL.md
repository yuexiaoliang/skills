...
- 误判为瞬态：最多浪费 19 分钟退避（0+1+3+5+10）后自动回退到 10.2，代价可控。
...
**理由**：瞬态错误（API 限流、网络抖动、服务端 5xx）通常在秒级到分钟级内自行恢复。首次检测到时，模型 / 网络很可能已经恢复，应直接尝试接续；只在连续失败时才逐步延长退避。

```bash
# 首次（consecutive_transient_count == 0）跳过 sleep
sleep <等待秒数>
